...
public class ConsultasDAOImp extends Database implements ConsultasDAO {
	...
	// Permite tener una cuenta de las órdenes que se encuentran pendientes en la
	// cocina.
	@Override
	...
	public ArrayList<Request> getRequest(Request request) {
		...
		try {
			...
			while (rs.next()) {
				...
				listRequest.add(new Request(request.getIdRequest(), request.getTotalDetail(), request.getTotalStatus(),
	...
	@Override
	public ArrayList<Request> getRequestFinished(Request request) { //NUEVA

		PreparedStatement ps = null;
		Connection con = getConexion();
		ResultSet rs = null;

		String sql = "SELECT idPedido from pedido where  estadoPedido = 'Terminado'";
